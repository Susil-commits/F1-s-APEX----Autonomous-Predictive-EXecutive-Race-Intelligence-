import React from 'react';
import { OfficialSponsor } from '../data/constants';

export interface GlobalPartnersProps {
  sponsors: OfficialSponsor[];
}

export const GlobalPartners: React.FC<GlobalPartnersProps> = ({ sponsors }) => {
  return (
    <div className="w-full pt-10 pb-6 border-t border-slate-200 dark:border-white/10 flex flex-col items-center gap-5">
      <span className="text-[11px] font-bold uppercase tracking-widest text-slate-400">
        Formula 1 Official Partners
      </span>

      <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 opacity-70 hover:opacity-100 transition-opacity">
        {sponsors.map((s) => (
          <img
            key={s.name}
            src={s.logo}
            alt={s.name}
            className="h-5 sm:h-6 w-auto object-contain filter grayscale dark:invert hover:grayscale-0 dark:hover:invert-0 transition-all duration-200"
          />
        ))}
      </div>

      <p className="text-[11px] text-slate-400 dark:text-slate-500 text-center mt-2 font-mono">
        © 2003–2026 Formula One World Championship Limited. APEX Autonomous Predictive Intelligence.
      </p>
    </div>
  );
};

export default GlobalPartners;
