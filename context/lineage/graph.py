...
Constructs and manages the directed acyclic graph (DAG) modeling context entities and relationships:
  Race -> Session -> Driver -> Team -> WeatherSource -> Tool ->
...
from context.metadata.model_metadata import MODEL_REGISTRY
from context.metadata.dataset_metadata import DATASET_REGISTRY
...
class RaceContextGraph:
    ...
    def get_edge(self, source_id: str, target_id: str) -> Optional[ContextEdge]:
        """Fetch directed edge between two nodes."""
        for edge in self._adj_out.get(source_id, []):
            if edge.target_id == target_id:
                return edge
        return None

    def get_upstream_lineage(self, node_id: str, max_depth: int = 10) -> List[ContextNode]:
        """Traverse incoming edges backwards to discover full upstream data, feature, and model provenance."""
        visited: Set[str] = set()
    ...
    def get_downstream_impact(self, node_id: str, max_depth: int = 10) -> List[ContextNode]:
    ...
    def build_lineage_chain(
        ...
        """Connects the explicit 7-stage APEX context lineage pipeline."""
        # 1. Telemetry -> FeatureSet
        self.add_edge(ContextEdge(
            source_id=telemetry_id,
            target_id=features_id,
            relation_type=RelationType.EXTRACTED_FROM,
            properties={"schema": "race_features_v3", "latency_ms": 0.0245}
        ))
        # 2. FeatureSet -> Model
        self.add_edge(ContextEdge(
            source_id=features_id,
            target_id=model_id,
            relation_type=RelationType.CONSUMED_BY,
            properties={"vector_dim": 28}
        ))
        # 3. Model -> Prediction
        self.add_edge(ContextEdge(
            source_id=model_id,
            ...
            properties={"ci_nominal": 0.95}
        ))
        # 4. Prediction -> StrategyCandidate
        self.add_edge(ContextEdge(
            source_id=prediction_id,
            target_id=candidate_id,
            relation_type=RelationType.INFORMS,
            properties={"influence_weight": 0.88}
        ))
        # 5. StrategyCandidate -> Counterfactual
        self.add_edge(ContextEdge(
            source_id=candidate_id,
            target_id=counterfactual_id,
            relation_type=RelationType.EVALUATED_BY,
            properties={"rollouts": 1000}
        ))
        # 6. Counterfactual -> Decision
        self.add_edge(ContextEdge(
            source_id=counterfactual_id,
            target_id=decision_id,
            relation_type=RelationType.LEADS_TO,
            properties={"utility_best": 0.82}
        ))
        # 7. Decision -> Outcome
        self.add_edge(ContextEdge(
        ...
        for u in upstream:
            if u.entity_type == EntityType.TELEMETRY_STREAM:
                ...
                models_invoked.append({
                    ...
                    "r2_score": u.properties.get("metrics", {}).get("r2", 0.8342),
                })
            elif u.entity_type in (EntityType.PREDICTION,):
                predictions_produced[u.name] = u.properties
            elif u.entity_type in (EntityType.COUNTERFACTUAL,):
                branches = u.properties.get("branches")
                if branches and isinstance(branches, list):
                    counterfactuals.extend(branches)
                else:
                    counterfactuals.append(u.properties)
            elif u.entity_type in (EntityType.SAFE_RL_GUARDRAIL, EntityType.TOOL):
...
def build_default_race_context_graph(
    ...
    """Factory creating the complete 14-entity Race Intelligence Context Graph."""
    graph = RaceContextGraph(graph_id=f"apex-context-{circuit_name.lower()}-lap-{lap}")

    # 1. Race Entity Node
    race_node = ContextNode(
        id=f"race:{circuit_name.lower()}_2023",
        ...
        properties={"circuit": circuit_name, "year": 2023, "total_laps": 52, "lap_distance_km": 5.891},
    )
    graph.add_node(race_node)

    # 2. Session Entity Node
    session_node = ContextNode(
        id=f"session:{circuit_name.lower()}_2023_r",
        name=session_name,
        entity_type=EntityType.SESSION,
        description="Official Grand Prix race timing and telemetry session",
    ...
    # 3. Team Entity Node
    team_node = ContextNode(
        id="team:mclaren_f1",
        name="McLaren Formula 1 Team",
        entity_type=EntityType.TEAM,
        description="Constructor team operational baselines and pit crew efficiency",
        properties={"pit_crew_avg_stop_s": 2.45, "engine": "Mercedes-AMG M15"},
    )
    graph.add_node(team_node)

    # 4. Driver Entity Node
    driver_node = ContextNode(
        id=f"driver:car_{car_id}",
        name=driver_name,
        entity_type=EntityType.DRIVER,
        description=f"Lead Race Driver for Car #{car_id}",
        properties={"car_id": car_id, "team": "McLaren", "p_current": 1, "compound": "MEDIUM", "tyre_age": lap - 1},
    )
    ...
    graph.add_edge(ContextEdge(source_id=team_node.id, target_id=driver_node.id, relation_type=RelationType.EMPLOYS))

    # 5. Weather Source Entity Node
    weather_node = ContextNode(
        id=f"weather:{circuit_name.lower()}_doppler_station",
        name=f"{circuit_name} High-Resolution Doppler Radar & Barometric Station",
        entity_type=EntityType.WEATHER_SOURCE,
        description="Track-side barometric sensor array and Doppler radar feed",
        properties={"rain_probability_next_5_laps": 0.72, "track_wetness_index": 0.35, "track_temp_c": 38.5, "air_temp_c": 24.0},
    )
    graph.add_node(weather_node)
    graph.add_edge(ContextEdge(source_id=weather_node.id, target_id=session_node.id, relation_type=RelationType.OBSERVED_DURING))

    # 6. Telemetry Stream Entity Node (Chain Step 1)
    telemetry_node = ContextNode(
        ...
        properties={"frequency_hz": 60, "format": "Pydantic F1TelemetryPacket", "quality_score": 99.8, "stream_url": "fastf1_live_telemetry_60hz"},
    ...
    # 7. Feature Set Entity Node (Chain Step 2)
    ...
    graph.add_edge(ContextEdge(source_id=weather_node.id, target_id=feature_node.id, relation_type=RelationType.FEEDS))

    # 8. Model Asset Nodes (Chain Step 3)
    for model_key, model_card in MODEL_REGISTRY.items():
        ...
        graph.add_edge(ContextEdge(source_id=feature_node.id, target_id=m_node.id, relation_type=RelationType.CONSUMED_BY))

    # 9. Prediction Entity Nodes (Chain Step 4)
    tyre_pred_node = ContextNode(
        ...
        prediction_provenance=PredictionProvenanceRecord(
            ...
            source_session=f"2024_{circuit_name}_R",
    ...
    # 10. Strategy Candidate Entity Node (Chain Step 5)
    ...
    # 11. Counterfactual Entity Node (Chain Step 6)
    cf_node = ContextNode(
    ...
    # 12. Tool / Guardrail Entity Node (Safe RL Guardrail / SHAP)
    guardrail_node = ContextNode(
        id="tool:safe_rl_action_mask_v2",
        name="Safe RL Dynamic Action Mask Guardrail",
        entity_type=EntityType.SAFE_RL_GUARDRAIL,
        description="Physical and sporting regulatory feasibility mask (8 discrete constraints)",
    ...
    # 13. Decision Entity Node (Chain Step 7)
    ...
    # 14. Outcome Entity Node (Chain Step 8)
